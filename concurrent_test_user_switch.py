...
async def switch_user(user_data_dir: str, file_lock: asyncio.Lock, active_users_lock: asyncio.Lock, active_users: set, headless=False):
    ...
    # **Do not clean user_data_dir here initially.** We'll check login status first.

    browser = None # Define browser for finally block
    try:
        async with async_playwright() as p:
            # 使用传入的 user_data_dir 启动浏览器
            # Ensure parent directory exists before launching
            parent_dir = os.path.dirname(user_data_dir)
            if parent_dir and not os.path.exists(parent_dir):
                 os.makedirs(parent_dir, exist_ok=True)
                 print(f"Created parent directory: {parent_dir}")

            print(f"[并发登录] 尝试使用数据目录 {user_data_dir} 启动浏览器 for {selected_user['username']}")
            browser = await p.chromium.launch_persistent_context(user_data_dir, headless=headless, args=["--disable-blink-features=AutomationControlled"])
            page = browser.pages[0] if browser.pages else await browser.new_page()
            print(f"[并发登录] 浏览器启动成功 for {selected_user['username']}")

            await page.goto("https://chat.deepseek.com/")
            await asyncio.sleep(random.uniform(2, 4)) # Wait for potential auto-login

            # **Check if already logged in**
            is_logged_in = False
            try:
                # Check for a reliable element indicating logged-in state (e.g., chat input)
                await page.wait_for_selector("textarea#chat-input", timeout=5000, state="visible")
                print(f"[并发登录] 检测到已登录状态 for {selected_user['username']} (找到 textarea)")
                is_logged_in = True
            except Exception:
                print(f"[并发登录] 未检测到登录状态 for {selected_user['username']}，将尝试登录。")
                is_logged_in = False

            if is_logged_in:
                # Already logged in, no need for login steps or cleaning dir
                print(f"[并发登录] 用户 {selected_user['username']} 已自动登录，跳过登录步骤。")
                await browser.close() # Close the browser
                browser = None
                return selected_user['username'] # Return username, login successful (implicitly)
            else:
                # **Not logged in, proceed with login steps**
                # **Clean the user data directory NOW before attempting login**
                # This requires closing the current context first.
                print(f"[并发登录] 需要登录，正在清理目录 {user_data_dir} 并重新启动浏览器 for {selected_user['username']}")
                await browser.close() # Close the initial context
                ...
                if os.path.exists(user_data_dir):
                    ...
                    try:
                        ...
                        print(f"[并发登录] 已清理 {user_data_dir} for {selected_user['username']}")
                    except OSError as e:
                        print(f"[并发登录] 清理 {user_data_dir} 时出错 (可能被占用): {e}")
                        # If cleanup fails, login might still work but could be flaky. Proceed with caution.

                # Relaunch the browser with the (now clean) user_data_dir
                print(f"[并发登录] 重新启动浏览器 for {selected_user['username']} after cleaning dir.")
                browser = await p.chromium.launch_persistent_context(user_data_dir, headless=headless, args=["--disable-blink-features=AutomationControlled"])
                page = browser.pages[0] if browser.pages else await browser.new_page()
                await page.goto("https://chat.deepseek.com/")
                ...
                # 2. 点击“切换到密码登录”
                login_form_selector = 'div.login_form_wrapper' # Use a more specific container if possible
                ...
                if await password_tab.count() > 0:
                     ...
                     print(f"[并发登录] 已点击'密码登录' for {selected_user['username']}")
                else:
                     print(f"[并发登录][WARN] 未找到'密码登录'按钮 for {selected_user['username']}")
                     # Consider raising an error or logging

                await asyncio.sleep(random.uniform(1,2))

                # 4. 输入账户
                username_input = page.locator('input[placeholder="手机号/邮箱"], input[name="account"]')
                await username_input.wait_for(state="visible", timeout=10000)
                await username_input.fill(selected_user['username'])
                await asyncio.sleep(random.uniform(0.5, 1))

                # 5. 输入密码
                password_input = page.locator('input[placeholder="密码"], input[name="password"]')
                ...
                print(f"[并发登录] 已点击登录 for {selected_user['username']}")

                # Wait for login success confirmation (e.g., textarea appears)
                await page.wait_for_selector("textarea#chat-input", timeout=30000, state="visible")
        ...
        if browser:
            try:
                await browser.close()
                print(f"[并发登录] Browser context closed in finally block for {selected_user['username']}")
            except Exception as close_err:
                 print(f"[并发登录][WARN] Error closing browser in finally block for {selected_user['username']}: {close_err}")
